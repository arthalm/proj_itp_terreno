...
int Terreno::obterAltura()
{
    return altura;
}

int Terreno::obterSemente()
{
    return sementeInicial;
}

int Terreno::obterAltMinimo()
{
    return altMinimo;
}

int Terreno::obterAltMaximo()
{
    return altMaximo;
}
...
Imagem Terreno::carregarTerreno(std::string arquivoPALETA, float fator, int dist, int map)
{
    ...
    for (int y = 0; y < altura; y++)
    {
        for (int x = 0; x < largura; x++)
        {
            ...
            int limiarAgua = min + (int)((max - min) * 0.28);

            if (y - dist >= 0 && x - dist >= 0 && alturaAtual > limiarAgua)
            {
