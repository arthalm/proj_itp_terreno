...
TEST_CASE("Testa a modificação e acesso as altitudes da matriz")
{
    ...
    terreno.inserirAltura(1, 1, 42);
    CHECK(terreno.obterAltura(1, 1) == 42);
}
...
TEST_CASE("Testa gerarMapa nos quatro cantos para terreno 2x2")
{
    ...
    CHECK(terreno.obterAltura(0, 0) >= 0);
    CHECK(terreno.obterAltura(0, 0) <= 100);

    CHECK(terreno.obterAltura(0, 2) >= 0);
    CHECK(terreno.obterAltura(0, 2) <= 100);

    CHECK(terreno.obterAltura(2, 0) >= 0);
    CHECK(terreno.obterAltura(2, 0) <= 100);

    CHECK(terreno.obterAltura(2, 2) >= 0);
    CHECK(terreno.obterAltura(2, 2) <= 100);
}
...
TEST_CASE("Testa gerarMapa com numero negativo")
{
    ...
    CHECK(terreno.obterAltura(0, 0) >= -10);
    CHECK(terreno.obterAltura(0, 0) <= 10);

    CHECK(terreno.obterAltura(0, 2) >= -10);
    CHECK(terreno.obterAltura(0, 2) <= 10);

    CHECK(terreno.obterAltura(2, 0) >= -10);
    CHECK(terreno.obterAltura(2, 0) <= 10);

    CHECK(terreno.obterAltura(2, 2) >= -10);
    CHECK(terreno.obterAltura(2, 2) <= 10);
}
...
TEST_CASE("Testa a salvamento do terreno em formato R16 (HeightMap)")
{
    ...
    terreno.inserirAltura(0, 0, 3);
    terreno.inserirAltura(0, 1, 0);
    terreno.inserirAltura(0, 2, 1);
    terreno.inserirAltura(1, 0, 0);
    terreno.inserirAltura(1, 1, 0);
    terreno.inserirAltura(1, 2, 0);
    terreno.inserirAltura(2, 0, 0);
    terreno.inserirAltura(2, 1, 0);
    terreno.inserirAltura(2, 2, 2);
...
TEST_CASE("Testa a leitura de terreno em formato R16 (HeightMap)")
{
    ...
    const int &p1 = terreno.obterAltura(0, 0);
    CHECK(p1 == 3);

    const int &p2 = terreno.obterAltura(0, 1); // (x,y) = (1,0)
    CHECK(p2 == 0);

    const int &p3 = terreno.obterAltura(0, 2); // (x,y) = (0,1)
    CHECK(p3 == 1);

    const int &p4 = terreno.obterAltura(1, 0);
    CHECK(p4 == 0);

    const int &p5 = terreno.obterAltura(1, 1);
    CHECK(p5 == 0);

    const int &p6 = terreno.obterAltura(1, 2);
    CHECK(p6 == 0);

    const int &p7 = terreno.obterAltura(2, 0);
    CHECK(p7 == 0);

    const int &p8 = terreno.obterAltura(2, 1);
    CHECK(p8 == 0);

    const int &p9 = terreno.obterAltura(2, 2);
    CHECK(p9 == 2);
}
...
TEST_CASE("Testa geração, visualização e salvamento de um terreno potencia 5")
{
    ...
    CHECK(terreno.obterAltura(0, 0) >= 0);
    CHECK(terreno.obterAltura(0, 0) <= 300);

    CHECK(terreno.obterAltura(32, 32) >= 0);
    CHECK(terreno.obterAltura(32, 32) <= 300);
...
TEST_CASE("Testa geração, visualização e salvamento de um terreno potencia 8")
{
    ...
    CHECK(terreno.obterAltura(256, 256) >= 0);
    CHECK(terreno.obterAltura(256, 256) <= 300);
